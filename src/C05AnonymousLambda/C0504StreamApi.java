...
////        java에서 함수형 프로그래밍을 지원하기 위한 라이브러리는 streamAPI
...
//        원시자료형을 가지고 stream 객체를 만들게 되면, Stream<T>가 아니라, 별도의 Stream 객체 존재
...
////        원시 자료형의 경우 Intstream으로 stream 객체가 반환되므로, 제네릭 타입소거 문제 발생하지 않음.
//        int[] newIntArr = Arrays.stream(intArr).filter(a->a<=30).toArray();
//        System.out.println(sum);
//        System.out.println(Arrays.toString(newIntArr));
//
//        String[] stArr = {"HTML","CSS","JAVA","PYTHON"};
////        제네릭 타입소거로 인해, .toArray를 통해 바로 새로운 String배열을 만드는것이 불가
////        제네릭 타입소거란 <String> 이러한 제네릭의 타입이 런타임시점에서는 제거되는것을 의미
...
//        Student s2 = new Student("choi",32);
//        Student s3 = new Student("lee",35);
//        Student s4 = new Student("park",22);
//        students.add(s1);
//        students.add(s2);
//        students.add(s3);
//        students.add(s4);
...
//
//        Student st1 = students.stream().sorted((o1,o2)->o1.getAge()-o2.getAge()).findFirst().get();
...
//            System.out.println("값이 없습니다");
//        }
////        방법2. orElse() : 값이 있으면 있는 return, 없으면 지정값 return
//        System.out.println(opt2.orElse("none"));
////        방법 3. orElseGet() : 값이 있으면 있는 값 return 없으면 람다함수 실행
////        방법 4. orElseThrow : 값이 있으면 있는 값 return 없으면 지정된 예외 강제 발생.
...
//        Student가 있으면 student 출력. 없으면 "해당학생은 없습니다." 라고 에러 메시지와 함께 에러 강제발생.
